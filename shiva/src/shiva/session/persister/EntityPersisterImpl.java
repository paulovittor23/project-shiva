...
public class EntityPersisterImpl implements EntityPersister {
	...
	/* (non-Javadoc)
	 * @see shiva.session.persister.EntityPersister#exists(java.lang.Object)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see shiva.session.persister.EntityPersister#getAll(java.lang.Class)
	 */
		...
		try {
			...
			while(rs.next()){
				...
				for( Field key : keySet ) {
					...
					//get relative field from entity class instance
	...
	private void executeUpdateSql( String sql ) {
		...
		try {
			...
			conn = factory.getConnection( this.config );*/
	...
	private ResultSet executeQuerySql( String sql ) {
		...
		try {
			/*ConnectionFactory factory = new ConnectionFactory();
			conn = factory.getConnection( config );*/
